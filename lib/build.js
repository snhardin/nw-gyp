...
function getCleanedEnvironment(sourceEnv) {
  const clonedSourceEnv = { ...sourceEnv };
  const cleanedEnv = {};
  const seenKeysNormalized = new Set();
  const duplicatesLog = [];
  const keysInOrder = Object.keys(clonedSourceEnv);

  for (const key of keysInOrder) {
    const value = clonedSourceEnv[key];
    const normalizedKey = key.toUpperCase();

    if (!seenKeysNormalized.has(normalizedKey)) {
      seenKeysNormalized.add(normalizedKey);
      cleanedEnv[key] = value;
    } else {
      let originallyKeptKey = "";
      for (const k in cleanedEnv) {
        if (Object.prototype.hasOwnProperty.call(cleanedEnv, k) && k.toUpperCase() === normalizedKey) {
          originallyKeptKey = k;
          break;
        }
      }
  ...
  function doBuild () {
    ...
    let cleanEnv = getCleanedEnvironment(process.env)
    var proc = gyp.spawn(command, argv, {env: cleanEnv})
    proc.on('exit', onExit)
  }
